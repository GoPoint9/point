...
import { isInput, inRange, getTouchEvent, blurActiveElement, windowWidthWithoutScrollbar, stringStylesToObject } from '../utils'
...
export default {
  ...
  props: {
    ...
    resizeEdges: {
      ...
      validator: val =>
        ['r', 'br', 'b', 'bl', 'l', 'tl', 't', 'tr'].filter(value => val.indexOf(value) !== -1).length === val.length,
      type: Array,
    },
    centerResize: {
      type: Boolean,
  ...
  computed: {
     ...
     */
    position () {
      const { viewportHeight, viewportWidth, shiftLeft, shiftTop, shiftX, shiftY, trueModalWidth, trueModalHeight } = this
  ...
  methods: {
    ...
    getResizedShiftLeft (event) {
      const { viewportHeight, viewportWidth, trueModalWidth, trueModalHeight } = this
