<template>
  <router-view v-if="['/agent', '/agentHome'].includes(route.path)" class="agent-main" name="agent" />
  <div v-else class="view-wrapper">
    <router-view name="SideMenu" />
    <div class="view-container">
      <router-view name="header" />
      <router-view :key="mainRouterKey" class="main-content" />
...
import CommonLogic from '@entrance/ncz-web-pgpc/components/CommonLogic'
...
import { GlobalStore, useSiteStore, useCommonStore, useUserStore } from '@/stores'
import { addMaximumScaleToMetaViewport, checkIsIOS, difflanguage, getBrowserLang, setHtmlLang } from '@/utils'
...
const isTabBar = computed(() => route.meta.tabBar)
const mainRouterKey = computed(() => {
  const result = ['/', '/index', '/login', '/register'].includes(route.path.value) ? 'index' : route.fullPath.value
  console.log(' route ', result, route.path.value)
  return result
})

const VITE_HOME = import.meta.env.VITE_HOME

console.warn('VITE_HOME ', VITE_HOME)
...
const $showSideMenu = computed(() => commonStore.showSideMenu)
provide('$showSideMenu', $showSideMenu)
window.setLoading = commonStore.setLoading
commonStore.$subscribe((mutation, state) => {
  // console.warn(' [commonStore.$subscribe] mutation, state ', mutation, state)
  // NOTE: 這邊再斟酌一下是有需要替換
  window.cover.loader(state.isLoading)
  // isLoad.value = state.isLoading
  commonStore.setLoading(state.isLoading)
})

const currentFontFamily = ref('bahnschrift')
const browerlang = getBrowserLang()
const languages = siteStore.getLanguage
...
// console.log('66666',siteStore.getAreacode)
// userStore.setNumberType(siteStore.getAreacode.substring(1))
fixLang()
onMounted(() => {
  if (checkIsIOS()) {
