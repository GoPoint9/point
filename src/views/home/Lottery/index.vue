<template>
    ...
    <div class="banner">
      <div class="insideBanner">
        <div class="Firstbanner">
          <span class="line" />
          <div class="insideFirstbanner">
            {{ $t('wingoLng') }}<br><span>{{ $t('lotteryResultFive') }}</span>
          </div>
        </div>
        <div class="Lastbanner">
          <div class="btnLottery" @click="goLotteryNew('AllLotteryGames-ChangLong')">{{ $t('dragonEntry') }} >></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div
    v-for="(item, index) in lotteryData.slice(0, 4)"
    :key="index"
    class="MainContainer"
    @click="goLottery(item.id)"
  >
      ...
      <div class="GoBtn">
        ...
        >
          <circle
            ...
            cy="33.1044"
            r="33"
            transform="rotate(-90 33.751 33.1044)"
            fill-opacity="0.2"
            fill="#304452"
          />
